#!/usr/bin/env python
"""
Скрипт для загрузки начальных данных в проект
"""
import os
import sys
import django
...
# Добавим путь к проекту в PYTHONPATH
sys.path.append('/var/www/online_school-1d4ddba33e53edf30d58b423fb3bb9f4df66367f/online_school-1d4ddba33e53edf30d58b423fb3bb9f4df66367f')
...
def load_initial_data():
    ...
    for parent_data in parents_data:
        if not User.objects.filter(username=parent_data['username']).exists():
            parent = User.objects.create_user(**parent_data)
            print(f"✅ Создан родитель: {parent.username}")
    
    # Создание курсов (если есть приложение courses)
    try:
        ...
        print("⚠️  Приложение courses не найдено, пропускаем создание курсов")
    
    # Создание платежей (если есть приложение payments)
    try:
        from payments.models import Payment
        
        if students.exists() and Course.objects.exists():
            courses = Course.objects.all()
        ...
        print("⚠️  Приложение payments не найдено, пропускаем создание платежей")
